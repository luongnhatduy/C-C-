...
{   
	...
	{ 
        y1=y;
        y2=y;
		while(getpixel(x,y1-1)!=color)
			y1--;
		while(getpixel(x,y2+1)!=color)
			y2++;
		for(int i=y1;i<=y2;i++)
			putpixel(i,y,tomau);
		while(getpixel(x+1,y1)==color)
			y1++;
		if(y1<=y2)
		{
			x = x=1;
...
void nhap(int & tam_x,int & tam_y,int & color, int &mauto)
{
	...
	cin>> color;
	cout<<"nhap gia tri mau to  la ";
	cin>> mauto;
}
 main()
{
    nhap(tam_x,tam_y,color,mauto);
    initwindow(640,480);
    Elipse(tam_x,tam_y,300,150,color);
    tomau(tam_x,tam_y,mauto,color);
    getch();
    return 0;
}
